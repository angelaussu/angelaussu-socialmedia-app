...
export default function CreatePostPage() {
  ...
  function onSubmit(data: CreatePostFormData) {
    ...
    mutate(formData, {
      ...
      onError: (err: unknown) => {
        ...
        dispatch(showAlert({ message: apiErr?.message || "Failed to create post", type: "error" }));
      },
    });
  }
  ...
  return (
    <div className="w-full max-w-[600px] mx-auto px-4">
      <Link href="/feed" className="flex items-center gap-2 text-neutral-400 hover:text-neutral-25 mb-6 transition-colors">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
        <div>
          <label className="text-sm-bold text-neutral-25 block mb-1.5">Image</label>
          <div
          ...
          >
            {preview ? (
              ...
              <>
                ...
                <p className="text-md-regular text-neutral-600">Click to upload an image</p>
              </>
            )}
            <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={handleImageChange} />
          </div>
          {errors.image && (
            <span className="text-sm-regular text-red-400 mt-1 block">{String(errors.image.message)}</span>
          )}
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          />
          {errors.caption && (
            <span className="text-sm-regular text-red-400">{errors.caption.message}</span>
          )}
        </div>
